...
public class AuthServiceImpl implements AuthService {
    ...
    public AuthResponse authenticate(LoginRequest request) {
        ...
        // Use username as subject (stable). If user logged in with email, we still issue token for username.
        String subject = userRepository.findByUsername(request.getUsernameOrEmail())
                .map(User::getUsername)
                .or(() -> userRepository.findByEmail(request.getUsernameOrEmail()).map(User::getUsername))
    ...
    public TokenValidationResponse validate(String bearerTokenOrRawToken) {
        if (bearerTokenOrRawToken == null || bearerTokenOrRawToken.isBlank()) {
